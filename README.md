...
Status: **V1 released** (`v1.0.0`), with all of Sprint 9–19's V1.1 work now complete on top of it. FastAPI backend, Postgres-backed persistence with Alembic migrations, an event-driven core, mock and real (Anthropic **and** OpenRouter) providers, an SSE-realtime Next.js dashboard, a real git-backed Workspace per company, and a sandboxed execution gate for automated checks. Sprint 9 shipped local CEO accounts + session auth. Sprint 10 split Role (template-owned position) from Employee (company-owned instance). Sprint 11 added a hireable CTO position and a CEO-facing "Hire Employee" flow — the CEO can hire multiple Employees per worker Role and configure each one's model and skill template independently. Sprint 12 added PM↔CTO planning: hand the PM a vague request and it discusses it with the CTO (bounded turns), asks the CEO to clarify anything it can't honestly answer itself, and produces a reviewable, versioned **Project Specification** — engineering never starts until the CEO approves it. Sprints 13–14 shipped the **CEO Workspace** (`/company/[id]`), a single server-derived view of what needs the CEO next. Sprint 15 made that Workspace's optional sections configurable per-Company widgets. Sprint 16 added a budgeted **Agent Harness** tool loop for the Engineer; Sprint 17 added bounded self-correction on top of it. Sprint 18 added deterministic **Project Memory** (PM-triggered recall). Sprint 19 added a third real provider (**OpenRouter**, multi-model including free-tier options), structured JSON logging with request/mission correlation IDs, load-smoke-verified operating limits, and a full deployment walkthrough (`docs/DEPLOYMENT.md`). See `CHANGELOG.md` for the full v1.1.0 feature list and `docs/KNOWN_ISSUES.md` for every accepted tradeoff and verified limit. The `v1.1.0` tag itself is cut only after hands-on CEO verification. Everything below describes what's runnable **today**.
...
`COMMANDER_PROVIDER=openrouter` (Sprint 19) routes through [OpenRouter](https://openrouter.ai) instead — same `SecretsProvider`/boot-check/model-override plumbing as Anthropic, but multi-model, including free-tier tool-capable options. `OPENROUTER_API_KEY` follows the same "set in `.env` or paste into Company Settings" pattern; `make verify-llm-openrouter` is its `make verify-llm` equivalent.
