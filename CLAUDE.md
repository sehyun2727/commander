...
apps/api/          FastAPI backend (Python 3.11+, async SQLAlchemy, SQLite)
  ...
  tests/           pytest suite (142 tests, 4 skipped without a Docker daemon)
...
Not built yet (do NOT add without an explicit sprint brief): auth, hosted/remote cloud runner (beyond the local Docker sandbox), deployment/Launch, multi-provider beyond Anthropic+mock.
